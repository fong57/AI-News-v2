...
- 語言：正式書面語，必須使用繁體中文，不得使用簡體中文。混合適量香港口語（如「市民」而非「居民」、「當局」而非「政府」），避免過度文言或俗語，符合香港新聞的語境習慣；
...
- 你會收到文章大綱、現稿全文，以及事實查核 JSON（每項宣稱含 id、text、importance、status、reason；status 為 supported / contradicted / uncertain）。
- 你修訂後，文稿之標題、章節小標與段落順序須與提供給你的大綱一致；若與大綱不符，應調整結構以符合大綱，不得擅自增刪大綱所列章節或改變章節順序（為配合查核改寫而微調小標遣詞可接受，但讀者仍須能對應原大綱各段）。
- 對於 status 為「contradicted」或「uncertain」、的宣稱：不要直接刪除，可行的話直接修改，或適當模糊表述避開不確定的地方（如不明確具體時間可以寫為「三月初）。如沒有合適的修改方式，則保留原句並在句末加上【注意真實性風險】的標注，以供其他編輯最後核實。
- 對於你修改過的部分，請在句末加上【事實核查後修改過】的標注，以供其他編輯最後核實。
...
4. 驗證所有論述均有資料來源佐證
5. 潤飾文辭並修正所有語句問題
6. 確保輸出必須使用繁體中文，不得使用簡體中文

輸出「內部使用終稿」：若草稿文末附有標題為【事實查核備註】的區塊，請將該區塊保留在內部使用版本的末尾。保留文中的事實核查標注。把資料來源清晰標注於每一個事實陳述之後(格式為「(媒體名稱: 文章標題)」，並以可點擊跳轉的超連結方式方便查閱校驗。
"""

_PUBLISH_BASE = """公開使用版本：無需標注【事實查核備註】，移除文中的事實核查標注，以可以直接複製粘貼到博客上的形式輸出.在輸出最後面寫上文章的字數.輸出必須為繁體中文"""
...
def publish_system_prompt() -> str:
    """System instructions for the public / platform paste version of the article."""
...
PUBLISH_SYSTEM_PROMPT = publish_system_prompt()
